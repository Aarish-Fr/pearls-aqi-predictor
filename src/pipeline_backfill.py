...
def run_backfill():
    ...
    # Bulk Insert
    logger.info("Initiating bulk insert to MongoDB...")
    try:
        result = collection.insert_many(backfill_history, ordered=False)
        logger.info(
            "Bulk insert complete. %d new rows written to MongoDB.",
            len(result.inserted_ids)
        )
    except BulkWriteError as bwe:
        inserted = bwe.details.get("nInserted", 0)
        skipped  = len(backfill_history) - inserted
        logger.warning(
            "Bulk insert completed with %d rows inserted and %d duplicates skipped.",
            inserted, skipped
        )
